...
ifdef ON_COCALC
  ...
  ifeq ($(COCALC_OPTION), anaconda)
    ...
    CREATE_ENV ?= $(CONDA_PRE) $(CONDA_EXE) env create -y -f $< $(CONDA_PACKAGES) -p $@
    UPDATE_ENV ?= $(CONDA_PRE) $(CONDA_EXE) env update -y -f $< $(CONDA_PACKAGES) -p $@
...
MICROMAMBA_CREATE_ENV = $(_MICROMAMBA) create -y -f $< $(CONDA_PACKAGES) -p $@
...
ifdef ON_COCALC
	...
	$(_RUN) python3 -m ipykernel install --user --name "phys-521" \
                                       --display-name "Python 3 (phys-521)"
ifeq ($(EDITABLE), true)
  PIP_INSTALL_ARGS += -e
endif

$(ENV_PATH): environment.yaml Makefile
	$(CREATE_ENV)
...
ifdef ON_COCALC
	...
	$(_RUN) sphinx-autobuild --re-ignore '_build|_generated|autoapi' $(DOCS) $(DOCS)/_build/html
endif
...
Variables:
   ...
   CONDA_PACKAGES: (- "$(CONDA_PACKAGES)")
                     Set to add additional packages to the conda/micromamba environment
                     that supplies python.  For example, you might use this if you do not
